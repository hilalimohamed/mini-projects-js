...
searchInp.addEventListener("keyup", () => {
  ...
  ar = content
    ...
    .map((data) => `<li onclick="plus(this)">${data}</li>`)
    .join("");
...
function toShowQue(index) {
  ...
  let div =
    `<div>` +
    questionArray[index].choices[0] +
    `</div>` +
    `<div>` +
    questionArray[index].choices[1] +
    `</div>` +
    `<div>` +
    questionArray[index].choices[2] +
    `</div>` +
    `<div>` +
    questionArray[index].choices[3] +
    `</div>`;
...
imgg.setAttribute("src", "images/db.jpg");
x = 0;
...
h1.addEventListener("keyup", () => {
  keyupOrTarget = h1.value.replace(/[^A-Z0-9]/gi, "").toLowerCase();
...
function downloadFunction() {
  ...
  let blob2 = new Blob([contentQuote + "____by__" + byContext], {
    type: "text/plain",
  });
...
function listenFunction() {
  ...
  speach = new SpeechSynthesisUtterance(
    `${quoteText.innerText}+by+${by.innerText}`
  );
...
document.addEventListener("contextmenu", (e) => {
  ...
  x = x > winWidth - contOffsetWidth ? winWidth - contOffsetWidth : x;

  y = y > winHeight - contOffsetHeight ? winHeight - contOffsetHeight : y;
...
function gettingData(city) {
  ...
  fetch(apii).then((resp) =>
    resp.json().then((det) => {
      let na = (document.querySelector(
        ".location span"
      ).innerText = `${det.name}, ${det.sys.country}`);
      let lwla = (document.querySelector(".temp .numb").innerText = Math.floor(
        det.main.temp
      ));
      let bhalha = (document.querySelector(".temp2 .numb2").innerText =
        Math.floor(det.main.feels_like));
      let lktaba = (document.querySelector(".weather").innerText =
        det.weather[0].description);
      let flmya = (document.querySelector(
        ".details2 span"
      ).innerText = `${det.main.humidity}%`);
      console.log(det);
      console.log(na);
      console.log(lwla);
      console.log(bhalha);
